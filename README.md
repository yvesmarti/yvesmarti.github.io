# 📍 49° 11' N · 0° 21' E

*Quelque part en Normandie, entre deux réunions et un fichier Excel récalcitrant,  
je dessine des cartes et j'automatise ce qui peut l'être.*

**Yves Marti** — Chargé de mission déchets & SIG  
[yvesmarti.fr](https://yvesmarti.fr) · [LinkedIn](https://linkedin.com/in/yvesmarti)

---

## 🗺️ Légende

| · | Domaine |
|---|---------|
| 🟢 | Cartographie & SIG |
| 🔵 | Données & tableaux de bord |
| 🟣 | Outils web |
| 🟠 | Gestion de projet · marchés publics |
| 🔴 | IA appliquée au quotidien |

---

## 📐 Territoire

**🟢 Cartographie & SIG**  
QGIS · Leaflet · analyse spatiale · cartes web · flux WMS/WFS · OpenStreetMap

**🔵 Données & tableaux de bord**  
Excel · Power Query · APIs ouvertes (BAN, INSEE, IGN) · open data territorial

**🟣 Outils web**  
HTML · CSS · JavaScript vanille · Jekyll · GitHub Pages

**🟠 Terrain & coordination**  
Marchés publics (CCTP, DQE, analyse d'offres) · pilotage de projets intercommunaux · dialogue élus / communes / prestataires

**🔴 IA comme outil de travail**  
Claude · LM Studio · prompting appliqué à l'analyse et au développement

---

## 🔵 Points d'intérêt

Ce dépôt héberge mon site et une collection d'outils construits pour un usage réel — pas des démos, des trucs qui servent :

| Outil | Zone |
|-------|------|
| 🗺️ [Isochrones via ORS](https://yvesmarti.fr/outils/isochrones_ORS.html) | Géomatique |
| 📍 [Géocodage d'adresses en masse](https://yvesmarti.fr/outils/geocodage_adresse_ban.html) | Géomatique |
| 🔍 [Extraction de données OSM](https://yvesmarti.fr/outils/extracteurosm.html) | Géomatique |
| 📸 [Visionneuse photos géolocalisées](https://yvesmarti.fr/outils/photomapviewer.html) | Géomatique |
| 📅 [Calendrier de collecte](https://yvesmarti.fr/outils/generateur_calendrier_collecte.html) | Déchets |
| 🗑️ [Implantation PAV souterrains](https://yvesmarti.fr/outils/implantation-pav.html) | Déchets |
| 📱 [Suivi collecte terrain (PWA)](https://yvesmarti.fr/outils/suivi/) | Déchets |
| 🔲 [Générateur QR codes](https://yvesmarti.fr/outils/qr-code.html) | Pratique |
| 🖊️ [Annotateur de carte](https://yvesmarti.fr/outils/annotateur-carte.html) | Pratique |

---

## 📅 Parcours

```
2008 ──── LEA Bordeaux · anglais / espagnol
2013 ──── Licence Géographie · SIG (MapInfo)
2015 ──── Master 2 Développement durable territorial · QGIS
2015 ──── CD Pyrénées-Atlantiques · mission SIG / PDIPR
     │
2015 ──── Biomasse Normandie · Observatoire des déchets normands
     │         10 ans · suivi de flux · valorisation biomasse
     │
2025 ──►  CDC Normandie Cabourg · déploiement équipements collecte
```

---

## 📊 Repères

```
~70 collectivités côtoyées
  10 ans d'expérience territoriale
   9 outils web publiés en accès libre
```

---

*Si un de ces outils vous est utile, c'est déjà bien.*
